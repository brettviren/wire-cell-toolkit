...
function() {
    ...
    // Like per_anode but skips pre_proc (CMMModifier, FrameMasking,
    // ChargeErrorFrameEstimator).  Use this when the input frame has
    // sequential channel numbers (e.g. from MagnifySource) rather than
    // real detector channel IDs, because ChargeErrorFrameEstimator calls
    // anode->channel(id) and crashes on unknown IDs.
    // MaskSlices falls back to nthreshold-based slicing when error_tag
    // traces are absent.
    per_anode_no_preproc(anode, pipe_type = "single") :: imgpipe(anode, pipe_type),
}
...
function(params, anode, field, n, rms_cuts=[])
  {
    ...
    channel_info: [
      ...
      // with bin indices baked at jsonnet eval time.  pdsp has a single
      // production frame size (daq.nticks=nf.nsamples=6000, no Resampler), so
      // OmniChannelNoiseDB::set_nsamples() rebuilds to the same size and the
      // bin indices remain physically correct.  Note also that the current
      // pdOneChannelNoise consumer (Protodune.cxx, gated on iplane==2)

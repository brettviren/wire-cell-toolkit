...
#   - reality='data': inserts a 512->500 ns Resampler before NF (bottom-drift
#     anodes only).
#   - l1sp_pd_mode='process': LASSO writeback active (production bottom path).
#   - No elecGain override — protodunevd/params.jsonnet carries its own gains.
#
# Determinism: SP output (wiener0, gauss0) is bit-identical run-to-run on
...
#
#   WCT_PDVD_DATA   directory holding the anode 0 raw input:
#                     protodune-orig-frames-anode0.tar.bz2
#                   Defaults to:
...
print("PDVD anode 0 NF+SP regression: PASS")
PY
}
